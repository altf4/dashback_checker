...
def do_dashbacks(playerstate: melee.PlayerState, controller: melee.Controller):
    global dashback_state
    global dashback_frame

    # Move to the middle of the stage if we've gotten too far away
    if abs(playerstate.position.x) > 40:
        controller.tilt_analog(melee.Button.BUTTON_MAIN, int(playerstate.x < 0), 0.5)
        return dashback_state

    if playerstate.action == melee.Action.STANDING:
        dashback_state = 0

    # Start a dashback
    if dashback_state == 0:
        # Randomly determine the speed of the dashback
        dashback_state = random.random()
        dashback_frame = 0

    if dashback_frame < 4:
        dashback_frame += 1
        tilt_amount = min(max(dashback_state * dashback_frame, 0), 1)
        controller.tilt_analog(melee.Button.BUTTON_MAIN, tilt_amount, 0.5)
        return dashback_state
...
while True:
    ...
    if gamestate.menu_state in [melee.Menu.IN_GAME, melee.Menu.SUDDEN_DEATH]:
        ...
        if discovered_port > 0:
            ...
            do_dashbacks(gamestate.players[discovered_port], controller)
        ...
        if gamestate.menu_state == melee.Menu.CHARACTER_SELECT:
            melee.menuhelper.MenuHelper.choose_character(
                melee.Character.FOX, gamestate, controller_opponent
            )
